...
var jsp = require("./parse-js"), slice = jsp.slice, member = jsp.member, PRECEDENCE = jsp.PRECEDENCE;
...
function ast_walker(ast){
    ...
    walkers = {
        ...
        "try": function(t, c, f){
            return ["try", walk(t), c !== null && typeof c != "undefined" ? [c[0], walk(c[1])] : null, f !== null ? walk(f) : null];
        },
        "throw": function(expr){
...
Scope.prototype = {
    ...
    get_mangled: function(name, newMangle){
        if (this.uses_eval || this.uses_with) {
            return name; // no mangle if eval or with is in use
        }
        var s = this.has(name);
        if (!s) {
            return name; // not in visible scope, no mangle
        }
        if (HOP(s.mangled, name)) {
            return s.mangled[name]; // already mangled in this scope
        }
        if (!newMangle) {
            return name; // not found and no mangling requested
        }
...
function ast_add_scope(ast){
    ...
    return with_new_scope(function(){
        ...
        var ret = w.with_walkers({
            ...
            "try": function(t, c, f){
                if (c) {
                    return ["try", walk(t), with_new_scope(function(){
...
function gen_code(ast, beautify){
    ...
    function parenthesize(expr){
        var gen = make(expr);
        return member(expr[0], slice(arguments, 1)) ? gen = "(" + gen + ")" : gen;
    }
    ...
    generators = {
        ...
        "try": function(tr, ca, fi){
            ...
            if (ca) {
                out.push("catch", "(" + ca[0] + ")", make(ca[1]));
            }
            if (fi) {
                out.push("finally", make(fi));
            }
        ...
        "new": function(ctor, args){
            return add_spaces(["new", parenthesize(ctor, "seq", "binary", "conditional", "assign") + args]);
        },
        "switch": function(expr, body){
        ...
        "conditional": function(co, th, el){
            return add_spaces([parenthesize(co, "assign"), "?", parenthesize(th, "seq"), ":", parenthesize(el, "seq")]);
        },
        "assign": function(op, lvalue, rvalue){
            if (op && op !== true) {
                op += "=";
...
function ast_mangle(ast, do_toplevel){
    ...
    return w.with_walkers({
        ...
        "try": function(t, c, f){
            return ["try", walk(t), c ? with_scope(c.scope, function(){
    ...
    return w.with_walkers({
        ...
        "if": function(c, t, e){
            ...
            if (t[0] == "stat") {
                ...
                if (e && t[0] == e[0] && (t[0] == "return" || t[0] == "throw")) {
        ...
        "block": function(body){
            if (body) {
                return ["block", join_vars(body.map(walk))];
            }
        },
        "binary": function(op, left, right){
            left = walk(left);
