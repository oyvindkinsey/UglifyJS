...
var jsp = require("./parse-js"), slice = jsp.slice, PRECEDENCE = jsp.PRECEDENCE;
...
function ast_walker(ast){
    ...
    walkers = {
        ...
        "try": function(t, c, f){
            return ["try", walk(t), c !== null ? [c[0], walk(c[1])] : null, f !== null ? walk(f) : null];
        },
        "throw": function(expr){
...
function gen_code(ast, beautify){
    ...
    generators = {
        ...
        "new": function(ctor, args){
            var ct = make(ctor);
            if (ctor[0] == "seq" || ctor[0] == "binary" || ctor[0] == "conditional") {
                ct = "(" + ct + ")";
            }
            args = args.length > 0 ? "(" + add_commas(args.map(make)) + ")" : "";
            return add_spaces(["new", ct + args]);
        },
        "switch": function(expr, body){
        ...
        "conditional": function(co, th, el){
            var the_then = make(th), the_else = make(el), the_cond = make(co);
            if (th[0] == "seq") {
                the_then = "(" + the_then + ")";
            }
            if (el[0] == "seq") {
            ...
            return add_spaces([the_cond, "?", the_then, ":", the_else]);
        },
        "assign": function(op, lvalue, rvalue){
            if (op && op !== true) {
                op += "=";
