...
var KEYWORDS = array_to_hash(["break", "case", "catch", "continue", "default", "delete", "do", "else", "finally", "for", "function", "if", "in", "instanceof", "new", "return", "switch", "throw", "try", "typeof", "var", "void", "while", "with", "NaN"]);

var RESERVED_WORDS = array_to_hash(["abstract", "boolean", "byte", "char", "class", "const", "debugger", "double", "enum", "export", "extends", "final", "float", "goto", "implements", "import", "int", "interface", "long", "native", "package", "private", "protected", "public", "short", "static", "super", "synchronized", "throws", "transient", "volatile"]);
...
function tokenizer($TEXT, skip_comments){
    ...
    function read_multiline_comment(){
        ...
        return with_eof_error("Unterminated multiline comment", function(){
            var i = find("*/", true), text = S.text.substring(S.pos, i), tok = token("comment2", text);
            S.pos = i + 2;
    ...
    handle_slash = skip_comments ? function(){
        next();
        switch (peek()) {
            case "/":
                read_line_comment();
...
function parse($TEXT, strict_semicolons, embed_tokens){
    ...
    function subscripts(expr, allow_calls){
        if (is("punc", ".")) {
            next();
            return subscripts(as_("dot", expr, as_name()), allow_calls);
        }
        if (is("punc", "[")) {
            next();
            return subscripts(as_("sub", expr, prog1(expression, curry(expect, "]"))), allow_calls);
        }
        if (allow_calls && is("punc", "(")) {
            next();
            return subscripts(as_("call", expr, expr_list(")")), true);
        }
    ...
    function array_(){
        return as_("array", expr_list("]", !strict_semicolons));
    }
    ...
    function function_(in_statement){
        ...
        expect("(");
        return as_(in_statement ? "defun" : "function", name, // arguments
 (function(first, a){
            while (!is("punc", ")")) {
                if (first) {
                    first = false;
 ...
 (function(a){
    ...
    function expr_atom(allow_calls){
        ...
        if (HOP(ATOMIC_START_TOKEN, S.token.type)) {
            var atom = S.token.type == "regexp" ? as_("regexp", S.token.value[0], S.token.value[1]) : as_(S.token.type, S.token.value);
    ...
    new_ = function(){
        ...
        return subscripts(as_("new", newexp, args), true);
    };
    ...
    function expr_op(left, min_prec){
        ...
        if (prec !== null && prec > min_prec) {
            ...
            return expr_op(as_("binary", op, left, right), min_prec);
        }
        return left;
    }
    ...
    function maybe_conditional(commas){
        ...
        if (is("operator", "?")) {
            ...
            var yes = expression();
            expect(":");
            return as_("conditional", expr, yes, expression(commas));
        }
        return expr;
    }
    ...
    function maybe_assign(commas){
        ...
        if (is("operator") && HOP(ASSIGNMENT, val)) {
            if (is_assignable(left)) {
                next();
                return as_("assign", ASSIGNMENT[val], left, maybe_assign(commas));
            }
    ...
    expression = function(commas){
        ...
        if (commas && is("punc", ",")) {
            next();
            return as_("seq", expr, expression());
        }
        return expr;
    };
    ...
    function labeled_statement(label){
        ...
        S.labels.pop();
        return as_("label", label, stat);
    }
    
    function simple_statement(){
        return as_("stat", prog1(expression, semicolon));
    }
    ...
    function for_(){
        ...
        if (is("name") && is_token(peek(), "operator", "in")) {
            ...
            var obj = expression();
            expect(")");
            return as_("for-in", has_var, name, obj, in_loop(statement));
        }
        else {
            // classic for
            ...
            expect(")");
            return as_("for", init, test, step, in_loop(statement));
        }
    }
    ...
    function try_(){
        ...
        return as_("try", body, bcatch, bfinally);
    }
    
    $statement = function(allow_case){
        switch (S.token.type) {
            case "num":
            case "string":
            case "regexp":
            ...
            case "keyword":
                switch (prog1(S.token.value, next)) {
                    ...
                    case "case":
                        ...
                        return as_("case", prog1(expression, curry(expect, ":")));
                    ...
                    case "do":
                        var a = (function(body){
                            ...
                            return as_("do", prog1(parenthesised, semicolon), body);
                        })(in_loop(statement));
                        return a;
                    case "for":
                        return for_();
                    ...
                    case "return":
                        ...
                        if (is("punc", ";")) {
                            ...
                            b = can_insert_semicolon() ? null : prog1(expression, semicolon);
                        }
                        return as_("return", b);
                    case "switch":
                        
                        var c = as_("switch", prog1(parenthesised, curry(expect, "{")), prog1(curry(in_loop, function(){
                            var a = [];
                            while (!is("punc", "}")) {
                        ...
                        return c;
                    case "throw":
                        return as_("throw", prog1(expression, semicolon));
                    ...
                    case "while":
                        return as_("while", parenthesised(), in_loop(statement));
                        
                    case "with":
                        return as_("with", parenthesised(), statement());
    ...
    statement = embed_tokens ? function(allow_case){
        var start = S.token;
 ...
 : $statement;
    ...
    return as_("toplevel", (function(a){
        while (!is("eof")) {
            a.push(statement());
        }
        return a;
